...
// --- Preload assets before revealing the solar system ---
...
function createLoaderUI() {
  ...
  wrap.innerHTML = `
    <style>
      #loader-wrap {
        ...
        background: linear-gradient(180deg, #0a0a12 0%, #0f0f1a 50%, #080810 100%);
      ...
      .loader-title {
        color: #fff; font-size: clamp(1.5rem, 4vw, 2.5rem); font-weight: 700;
        letter-spacing: 0.2em; text-transform: uppercase; margin-bottom: 0.5rem;
        text-shadow: 0 0 20px rgba(255,200,100,0.4);
      }
      .loader-sub { color: #889; font-size: 0.85rem; margin-bottom: 2rem; }
      .loader-bar-wrap {
        width: min(280px, 85vw); height: 8px;
        background: rgba(255,255,255,0.06); border-radius: 4px;
        overflow: hidden; box-shadow: inset 0 1px 2px rgba(0,0,0,0.5);
      }
      .loader-bar {
        height: 100%; width: 0%; background: linear-gradient(90deg, #f5c542, #ffdd99);
      ...
      .loader-frame {
        position: relative; padding: 2rem; border: 2px solid rgba(245,197,66,0.25);
        box-shadow: 0 0 30px rgba(245,197,66,0.08), inset 0 0 30px rgba(0,0,0,0.3);
      }
      .loader-frame::before, .loader-frame::after {
        content: ''; position: absolute; width: 16px; height: 16px;
        border: 2px solid rgba(245,197,66,0.5); pointer-events: none;
      }
      ...
      .loader-bar { animation: loader-pulse 1.5s ease-in-out infinite; }
      @keyframes loader-pulse { 0%,100% { opacity: 1; } 50% { opacity: 0.85; } }
      .loader-instructions {
        color: #ccc; font-size: 0.9rem; line-height: 1.6; text-align: center;
        max-width: 320px; margin: 1.5rem 0 1.25rem;
      }
      .loader-instructions ul { margin: 0.5rem 0; padding-left: 1.25rem; text-align: left; }
      .loader-instructions li { margin-bottom: 0.35rem; }
      .loader-btn {
        margin-top: 1rem; padding: 0.65rem 2rem; font-size: 1rem; font-weight: 600;
        background: linear-gradient(180deg, #f5c542, #d4a030); color: #1a1a1a;
        border: none; border-radius: 6px; cursor: pointer; letter-spacing: 0.05em;
  ...
  return {
    ...
    ready() {
      wrap.classList.add('loader-ready');
    ...
    setStatus(text) {
      ...
      this.wrap.classList.add('done');
      setTimeout(() => this.wrap.remove(), 700);
    }
  };
}
...
function runAfterPreload() {
  ...
  loader.btn.addEventListener('click', startExperience);
